...
Reproducible machine-learning code for J-PARC E90 analysis.
...
The recommended `v1` pipeline trains a multilayer perceptron to separate the `SigmaNCusp` signal from the `QFLambda` and `QFSigmaZ` backgrounds. Its 12 inputs are the `u_x`, `u_y`, `u_z`, and `dE/dx` variables for tracks `t0`, `t1`, and `t2`.
...
By default, input files are read from `./data/input` and generated data products are written below `./data/output`:
...
On KEKCC, point the project to the shared data area instead of copying large ROOT files into the repository:
...
Personal YAML files are ignored by Git.
...
The final test output is written as a ROOT `TTree` for compatibility with the existing ROOT analysis macros.

## Reproducible demo artifacts

The repository retains the compact artifacts needed to inspect or continue the documented `ptep_demo` run without repeating every expensive stage:
...
Each directory contains its configuration, executable code, reproduction instructions, and a technical report. The exact report databases and numerical summaries are retained, while `*_demo.yaml` files use non-conflicting output names for a fresh run.
...
If you use this code or the analysis method in your work, please cite the
corresponding paper:
...
@article{Amemiya2026PTEP,
  ...
  doi           = {10.48550/arXiv.2606.22750},
  url           = {https://arxiv.org/abs/2606.22750}
}
```
